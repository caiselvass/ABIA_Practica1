...
def generate_initial_state(n_furgonetas: int, greedy=False) -> EstadoBicing:
    ...
    if not greedy:
        ...
        for furgoneta in lista_furgonetas:
            ...
            id_est_o = random.randint(0, n_estaciones - 1)
            while id_est_o in est_con_furgoneta:
                id_est_o = random.randint(0, n_estaciones- 1)
            est_con_furgoneta.add(id_est_o)
            info_est_origen = info_estaciones[id_est_o]
            ...
            id_est_d1 = random.randint(0, n_estaciones - 1)
            id_est_d2 = random.randint(0, n_estaciones - 1)
            info_est_destino1 = info_estaciones[id_est_d1]
            info_est_destino2 = info_estaciones[id_est_d2]
            ...
            if num_bicicletas_carga_inicial < 0:
                num_bicicletas_carga_inicial = 0
            furgoneta.set_num_bicicletas_cargadas(num_bicicletas_carga_inicial)
                
        state = EstadoBicing(info_estaciones=info_estaciones, lista_furgonetas=lista_furgonetas)
        for furgoneta in state.lista_furgonetas:
            state.realizar_ruta_furgoneta(furgoneta.id)
        return state
...
if __name__ == '__main__':
    ...
    * Datos globales de bicicletas demandadas, disponibles para mover
    ...
    # Experimento
    initial_state: EstadoBicing = generate_initial_state(n_furgonetas=params.n_furgonetas, greedy=True)
    ...
    print("SOLUTIONS CHECKED:", problema_bicing.solutions_checked)
